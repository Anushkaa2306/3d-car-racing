...
let coin = { x: 200, y: 0, w: 30, h: 30, speed: 3 };
let gameOver = false;
...
// Draw coin
function drawCoin() {
    let img = document.getElementById("cn");
    ctx.drawImage(img, coin.x, coin.y, coin.w, coin.h);
}
...
// Example: end game after 20 seconds
...
(function () {
    ...
    if (h > w) {
        ...
        document.querySelector(".nav").style.transform = "translateX(" + (h) + "px) rotate(90deg)";
        document.getElementById("scoreDiv").style.transform = "translateX(" + (h) + "px) rotate(90deg)";
    } else {
        document.getElementById("mainContainer").style.transform = "";
        document.querySelector(".nav").style.transform = "";
        document.getElementById("scoreDiv").style.transform = "";
    }
    ...
    function loadGame() {
        ...
        for (var n = 0; n < ((h * .7 + 100) / gameDifficulty); n++) {
...
CarBuilder.prototype.draw = function () {
    ...
    this.y -= this.dy; //  for cars move UP (forward)





    // base lane calculation
    ...
    // make cars follow your car's forward direction smoothly

    let roadInfluence = (cx - w / 2) * 0.2; // how much road curves with you

    this.x += roadInfluence;



    //  smooth side drift so cars feel alive

    this.x += Math.sin(this.y / 40 + this.lane) * 1.5;
    ...
    if (this.y <= -100) { // if car goes out from top

        this.y = h + 100; // respawn from bottom behind you
...
document.getElementById("c1").style.width = "550px";
